...
int main(int argc, char *argv[])
{
    ...
    if (!canLaunch) {
        ...
        NSString *title = [NSString stringWithFormat:@"This version of %@ requires Mac OS X %@", applicationName, requiredVersion];
        ...
        if (sameMajorMinor) {
            messageFormat = [NSString stringWithFormat:@"Please use Software Update to upgrade to Mac OS X %@ or later so that you can use this version of %@.", requiredVersion, applicationName, nil];
            defaultButton = @"Software Update...";
            alternateButton = @"Quit";
        } else {
            messageFormat = [NSString stringWithFormat:@"Please install Mac OS X %@ or later so that you can use this version of %@. If you can't install the required version of Mac OS X then choose the \"Find earlier version...\" button to find a version of %@ that is compatible with your system.", requiredVersion, applicationName, applicationName, nil];
            defaultButton = [NSString stringWithFormat:@"Find earlier version of %@...", applicationName];
            alternateButton = @"Quit";
        }
        ...
        if (choice == NSAlertDefaultReturn) {
			if (sameMajorMinor) {
				...
				NSString *applicationVersionFeed = [[[mainBundle infoDictionary] objectForKey:@"SUFeedURL"] stringByDeletingPathExtension]; // delete .rss
				[[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:applicationVersionFeed]];
			}
        }
